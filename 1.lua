...
local function GetFormattedItemSections(itemSourceTable, isNewItems)
    ...
    if totalDust > 0 then
        local dustText
        if isNewItems then
            dustText = string.format("+ [%d] Dust", totalDust)
        else
            dustText = string.format("- Dust: %s", CommaFormat(totalDust))
        end
...
Remotes.UpInventory.OnClientEvent:Connect(function(category, data)
    ...
    elseif category == "Sword" or category == "Melee" then
        ...
        if Options.SelectedPassive then
            Options.SelectedPassive:SetValues(Tables.AllOwnedWeapons)
        end
    end
end)
...
Remotes.ArtifactSync.OnClientEvent:Connect(function(data)
    ...
    if DustLabel then
        DustLabel:SetText("Dust: " .. CommaFormat(data.Dust))
    end
    if InvLabel_Helmet then
        InvLabel_Helmet:SetText("Helmet: " .. counts.Helmet .. "/500")
    end
    if InvLabel_Gloves then
        InvLabel_Gloves:SetText("Gloves: " .. counts.Gloves .. "/500")
    end
    if InvLabel_Body then
        InvLabel_Body:SetText("Body: " .. counts.Body .. "/500")
    end
    if InvLabel_Boots then
        InvLabel_Boots:SetText("Boots: " .. counts.Boots .. "/500")
    end
end)
...
if Remotes.EnchantResult then
    Remotes.EnchantResult.OnClientEvent:Connect(HandleUpgradeResult)
end
if Remotes.BlessingResult then
    Remotes.BlessingResult.OnClientEvent:Connect(HandleUpgradeResult)
end
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = { {
            ...
            ["footer"] = {
                ["text"] = string.format("zeus • Session: %s • %s", GetSessionTime(), os.date("%x %X"))
            },
            ["thumbnail"] = { ["url"] = catLink }
        } }
    }
    
    if Toggles.PingUser.Value then
        payload["content"] = (Options.UID.Value ~= "" and "<@" .. Options.UID.Value .. ">" or "@everyone")
    end
...
local function CreateSwitchGroup(tab, id, displayName, tableSource)
    ...
    tab:AddDropdown("Default"..id, {
        Text = "Select Default "..displayName,
        Values = listToUse,
        Searchable = true
    })
    tab:AddDropdown(id.."_Mob", {
        Text = displayName.." [Mob]",
        Values = listToUse,
        Searchable = true
    })
    tab:AddDropdown(id.."_Boss", {
        Text = displayName.." [Boss]",
        Values = listToUse,
        Searchable = true
    })
    tab:AddDropdown(id.."_Combo", {
        Text = displayName.." [Combo F Move]",
        Values = listToUse,
        Searchable = true
    })
    tab:AddDropdown(id.."_BossHP", {
        Text = displayName.." [Boss HP%]",
        Values = listToUse,
        Searchable = true
    })
...
local function CheckPlayerForSafety(targetPlayer)
    ...
    if kickTypes["Mod"] then
        local success, rank = pcall(function()
            return targetPlayer:GetRankInGroup(TargetGroupId)
        end)
...
local function UpdateNPCLists()
    ...
    local currentList = {}
    for _, name in pairs(Tables.MobList) do
...
local function UpdateWeaponRotation()
    ...
    if tick() - Shared.LastWRSwitch >= switchDelay then
        ...
        if Shared.WeapRotationIdx > #weaponList then
    ...
    for _, name in ipairs(weaponList) do
        if name == Shared.ActiveWeap then
            exists = true
            break
        end
    end
...
local function IsMainStatGood(data, mainStatFilter)
    if data.Category == "Helmet" or data.Category == "Gloves" then
        return true
    end
...
local function EvaluateArtifact2(uuid, data)
    ...
    if Toggles.ArtifactLock.Value and not data.Locked and data.Level >= (lockMinSS * 3) then
        if IsWhitelisted(Options.Lock_MS.Value, data.MainStat.Stat)
           and IsWhitelisted(Options.Lock_Type.Value, data.Category)
           and IsWhitelisted(Options.Lock_Set.Value, data.Set) then
...
local function HandleSummons()
    ...
    local function IsSummonable(name)
        ...
        for _, boss in ipairs(Tables.SummonList) do
            if MatchName(boss, cleanName) then
                return true
            end
        end
...
local function UpdateSwitchState(target, farmType)
    ...
    for _, switch in ipairs(types) do
        ...
        if not (toggleObj and toggleObj.Value) then
            -- não está ativo, passa pro próximo
        else
            if switch.id == "Build" and tick() - Shared.LastBuildSwitch < 3.1 then
                -- cooldown de build, não troca agora
            else
                local toEquip = ""
                ...
                if farmType == "None" then
                    toEquip = Options["Default"..switch.id].Value
                elseif farmType == "Mob" then
                    toEquip = Options[switch.id.."_Mob"].Value
                elseif farmType == "Boss" then
                    if isLow then
                        toEquip = Options[switch.id.."_BossHP"].Value
                    else
                        toEquip = Options[switch.id.."_Boss"].Value
                    end
                end

                if toEquip and toEquip ~= "" and toEquip ~= "None" then
                    local finalEquipValue = toEquip
                    if switch.id == "Title" and toEquip:find("Best ") then
                        local bestId = GetBestOwnedTitle(toEquip)
                        if bestId then
                            finalEquipValue = bestId
                        else
                            finalEquipValue = nil
                        end
                    end

                    if finalEquipValue and finalEquipValue ~= Shared.LastSwitch[switch.id] then
...
local function UniversalPuzzleSolver(puzzleType)
    ...
    for i, islandOrPiece in ipairs(piecesToCollect) do
        ...
        if puzzleType == "Demonite" or puzzleType == "Hogyoku" then
            ...
            piece = islandFolder and islandFolder:FindFirstChild(pieceModelName, true)
                 or workspace:FindFirstChild(pieceModelName, true)
        end
                ...
                if currentStage then
                    ...
                    elseif taskType:find("BossKills") or taskType == "AnyBossKills" then
                        if taskType == "AnyBossKills" then
                            ...
                            if bossName == "" then
                                bossName = taskType:gsub("BossKills", "")
                            end

                            local lowerB = bossName:lower()
                            if lowerB:find("strongest") then
                                if lowerB:find("history") then
                                    bossName = "StrongestHistory"
                                elseif lowerB:find("today") then
                                    bossName = "StrongestToday"
                                end
                            end
                    ...
                    elseif taskType:find("Piece") or taskType:find("Found") then
                        local pType = "Dungeon"
                        if taskType:find("Slime") then
                            pType = "Slime"
                        elseif taskType:find("Demonite") then
                            pType = "Demonite"
                        elseif taskType:find("Hogyoku") then
                            pType = "Hogyoku"
                        end
...
local function GetAllMobTarget()
    ...
    if Shared.AllMobIdx > #rotateList then
        Shared.AllMobIdx = 1
    end
...
local function GetLevelFarmTarget()
    ...
    if not PGui.QuestUI.Quest.Visible then
        return nil
    end
    
    local questData = Modules.Quests.RepeatableQuests[Shared.QuestNPC]
    if not questData or not questData.requirements[1] then
        return nil
    end
    ...
    local bestMob, _count = GetBestMobCluster(matches)
...
local function GetOtherTarget()
    ...
    for _, npc in pairs(PATH.Mobs:GetChildren()) do
        ...
        if lowerSelected:find("strongest") then
            if name:find("strongest")
               and ((lowerSelected:find("history") and name:find("history"))
                 or (lowerSelected:find("today")   and name:find("today"))) then
                isMatch = true
            end
...
local function GetWorldBossTarget()
    ...
    if Toggles.BossesFarm.Value then
        ...
        for bossDisplayName, isEnabled in pairs(selected) do
            if isEnabled then
                for _, npc in pairs(PATH.Mobs:GetChildren()) do
                    if IsStrictBossMatch(npc.Name, bossDisplayName)
                       and not table.find(Tables.MiniBossList, npc.Name) then
                        if IsValidTarget(npc) then
...
local function GetMobTarget()
    ...
    for mob, enabled in pairs(selectedDict) do
        if enabled then
            table.insert(enabledMobs, mob)
        end
    end
    table.sort(enabledMobs)

    if #enabledMobs == 0 then
        return nil
    end

    if Shared.MobIdx > #enabledMobs then
        Shared.MobIdx = 1
    end
    
    local targetMobName = enabledMobs[Shared.MobIdx]
    local target, _count = GetBestMobCluster({[targetMobName] = true})
...
local function ShouldMainWait()
    if not Toggles.AltBossFarm.Value then
        return false
    end
...
local function GetAltHelpTarget()
    ...
    if not targetBossName then
        return nil
    end

    local targetNPC = nil
    for _, npc in pairs(PATH.Mobs:GetChildren()) do
        if IsStrictBossMatch(npc.Name, targetBossName) and IsValidTarget(npc) then
            targetNPC = npc
            break
        end
    end
...
local function Func_AutoM1()
    while task.wait(Options.M1Speed.Value) do
        ...
        if not Toggles.AutoM1.Value then
            break
        end
    end
end
    ...
    while task.wait() do
        if not Toggles.AutoSkill.Value then
            ...
            if canExecute and target and target.Parent then
                if target:FindFirstChild("IK_Active")
                   and Options.InstaKillType.Value == "V1"
                   and Toggles.InstaKill.Value then
                    canExecute = false
                end
            end
...
local function Func_AutoCombo()
    ...
    while Toggles.AutoCombo.Value do
        ...
        if rawPattern and rawPattern ~= "" then
            Shared.ParsedCombo = {}
            ...
            if #Shared.ParsedCombo > 0 then
                ...
                if IsBusy() then
                    ...
                    repeat
                        task.wait(0.1)
                    until not IsBusy() or (tick() - waitStart > 8)
                end
                task.wait(0.4) 

                local bossOnly = Toggles.ComboBossOnly.Value
                if bossOnly then
                    if not Shared.Target
                       or not Shared.Target.Parent
                        ...
                        if waitTime then
                            ...
                            if IsSkillReady(currentAction) then
                                if currentAction == "F" then
                                    ...
                                    local slotMap = {["Z"] = 1, ["X"] = 2, ["C"] = 3, ["V"] = 4}
                                    local slot = slotMap[currentAction] or 1
...
local function AutoRollStatsLoop()
    ...
    local hasRank = false
    for _ in pairs(selectedRanks) do
        hasRank = true
        break
    end
...
local function Func_UnifiedRollManager()
    while task.wait() do
        if Toggles.AutoTrait.Value then
            local traitUI = PGui:WaitForChild("TraitRerollUI").MainFrame.Frame.Content
                .TraitPage.TraitGottenFrame.Holder.Trait.TraitGotten
            local confirmFrame = PGui.TraitRerollUI.MainFrame.Frame.Content
        ...
        elseif Toggles.AutoRace.Value then
        ...
        elseif Toggles.AutoClan.Value then
    ...
    while Toggles.AutoSpec.Value do
        ...
        for weaponName, isWeaponEnabled in pairs(targetWeapons) do
            if isWeaponEnabled then
                ...
                if isCorrectName and type(currentBuffs) == "table" then
                ...
                if not isCorrectName or not meetsAllStats then
                    ...
                    repeat 
                        ...
                        local checkName = (type(checkData) == "table" and checkData.Name)
                            or (type(checkData) == "string" and checkData)
                            or ""
...
local function Func_AutoInfiniteTower()
    while Toggles.AutoInfiniteTower.Value do
        ...
        if isInTower then
            if Shared.AltActive or not Shared.Farm or Shared.MerchantBusy then
                -- pula essa iteração
            else
                local char = GetCharacter()
                local root = char and char:FindFirstChild("HumanoidRootPart")
...
local function Func_AutoMerchant()
    ...
    local function StartPurchaseSequence()
        ...
        if MerchantUI.MainFrame then
            MerchantUI.MainFrame.Visible = false
        end
    ...
    local function SyncClock()
        ...
        if Label and Label.Text:find(":") then
            ...
            if serverSecs then
                Shared.LocalMerchantTime = serverSecs
            end
        end
        if MerchantUI.MainFrame then
            MerchantUI.MainFrame.Visible = false
                ...
                if enabled then
                    ...
                    if Shared.TradeState.myItems then
                        for _, tradeItem in pairs(Shared.TradeState.myItems) do
                            if tradeItem.name == itemName then
                                alreadyInTrade = true
                                break
                            end
                        end
...
local function Func_AutoCraft()
    while Toggles.AutoCraftItem.Value do
        task.wait(1)
        ...
        if not Shared.ArtifactSession.Inventory or not next(Shared.ArtifactSession.Inventory) then 
            ...
            if #deleteQueue > 0 then
                for i = 1, #deleteQueue, 50 do
                    local chunk = {}
                    for j = i, math.min(i + 49, #deleteQueue) do
                        table.insert(chunk, deleteQueue[j])
                    end
            ...
            if #upgradeQueue > 0 then
                for i = 1, #upgradeQueue, 50 do
                    local chunk = {}
                    for j = i, math.min(i + 49, #upgradeQueue) do
                        table.insert(chunk, upgradeQueue[j])
                    end
            ...
            if Toggles.ArtifactEquip.Value then
                AutoEquipArtifacts()
            end
        end
    end
...
local Window = Library:CreateWindow({
	Title = "zeus",
	Footer = "" .. assetName .. " | by zeus | FreeDrop",
	NotifySide = "Right",
...
local function GetData()
    ...
    local tier = "<font color='#FFD700'>BY ZEUS https://discord.gg/freedrop</font>"
...
for k in pairs(ShortMap) do
    table.insert(SortedShortKeys, k)
end
...
Toggles.AutoEnchant:OnChanged(function(s)
    Thread("AutoEnchant", SafeLoop("Enchant", function() AutoUpgradeLoop("Enchant") end), s)
end)
Toggles.AutoEnchantAll:OnChanged(function(s)
    Thread("AutoEnchantAll", SafeLoop("EnchantAll", function() AutoUpgradeLoop("Enchant") end), s)
end)
Toggles.AutoBlessing:OnChanged(function(s)
    Thread("AutoBlessing", SafeLoop("Blessing", function() AutoUpgradeLoop("Blessing") end), s)
end)
Toggles.AutoBlessingAll:OnChanged(function(s)
    Thread("AutoBlessingAll", SafeLoop("BlessingAll", function() AutoUpgradeLoop("Blessing") end), s)
end)
...
Connections.Player_General = RunService.Stepped:Connect(function()
    ...
    if Hum then
        if Toggles.WS.Value then
            Hum.WalkSpeed = Options.WSValue.Value
        end
        if Toggles.JP.Value then
            Hum.JumpPower = Options.JPValue.Value
            Hum.UseJumpPower = true
        end
        if Toggles.HH.Value then
            Hum.HipHeight = Options.HHValue.Value
        end
    end
    workspace.Gravity = Toggles.Grav.Value and Options.GravValue.Value or 192
    if Toggles.FOV.Value then
...
task.spawn(function()
    while true do
        ...
        if Shared.AltActive then
            ...
            if not Shared.Farm or Shared.MerchantBusy or not Shared.Target then
                -- nada
            else
                local success, err = pcall(function()
                    local char = GetCharacter()
...
task.spawn(function()
    while task.wait() do
        if not Shared.Farm or Shared.MerchantBusy then 
            ...
            local char = GetCharacter()
            if char and not Shared.Recovering then
                if Shared.TargetValid
                   and (not Shared.Target or not Shared.Target.Parent or Shared.Target.Humanoid.Health <= 0) then
                    Shared.KillTick = tick()
                    Shared.TargetValid = false
                end

                if tick() - Shared.KillTick >= (tonumber(Options.TargetTPCD.Value) or 0) then
                    HandleSummons()
                        ...
                        for i = 1, #PriorityTasks do
                            ...
                            if taskName then
                                if isPityReady and (taskName == "Boss" or taskName == "All Mob Farm" or taskName == "Mob") then
                                    -- ignora essas tasks quando pity pronto
                                else
...
ThemeManager:SetFolder("zeus")
SaveManager:SetFolder("zeus/SailorPiece")
...
task.spawn(function()
    ...
    while not Shared.InventorySynced and timeout < 5 do
        ...
        if timeout == 1.5 and Remotes.ReqInventory then
            Remotes.ReqInventory:FireServer()
        end
    end
